# -*- coding: utf-8 -*-
"""
Created on Mon Mar 11 14:12:28 2024

@author: sstowe

Simulating Burgess 2010 Ultradian Protocol
0.5 mg dose of exogenous melatonin 
...
#----------- Load Burgess 2010 PRC data -------------
# From WebPlotDigitizer (n = 34)

Burgess_2010_PRC = [-0.9,
                    -0.3,
                    0.7,
                    1.4,
                    1.7,
...
#--------------- Plotting with Burgess Curve -----------

Burgess2010_fittedcurve_x = [0.107,
                             0.336,
                             0.565,
                             0.902,
                             1.27,
...
Burgess2010_fittedcurve_y = [0.325,
                             0.179,
                             0.0588,
                             -0.086,
                             -0.248,
...
plt.plot(Burgess2010_fittedcurve_x,Burgess2010_fittedcurve_y,'-',color='grey',lw=3)
plt.plot(Burgess_2010_PRC_times, Burgess_2010_PRC, 'o',color='black')
plt.xticks(np.arange(0, 26, 4))
plt.title("Simulated 3 Pulse PRC to 0.5 mg Melatonin")
plt.xlabel("Time After DLMO (hours)")
...
plt.plot(model.ts,model.results[:,3]/4.3,lw=2,color='mediumblue')
plt.plot(model.ts,model.results[:,4]/4.3,lw=2,color='mediumorchid')
plt.plot(model.ts,model.results[:,5]/4.3,lw=2,color='hotpink')
...
plt.plot(model.ts[241:480],model.results[241:480,3]/4.3,lw=3,color='mediumblue')
plt.plot(model.ts[241:480],model.results[241:480,4]/4.3,lw=3,color='mediumorchid')
#plt.plot(model.ts[241:480],model.results[241:480,5]/4.3,lw=2)
plt.axhline(DLMO_threshold)
#plt.ylim(0, 500)
plt.axvline(check+24,color='grey')
